...
layout = [  [sg.Text('Aventura Solo: Uma Luz na Escuridão \n' \
            ...
            [sg.Output(size=(130,25),font='Times 14 italic bold')], # Anything you 'print' will show upo here
            [sg.Text('Escolha: ')],
...
event=0             #Chaves que dão acesso às partes da aventura
inventario = []     #Inventário: Itens que podem ser usados durante a aventura
caminho = []        #Sequência de escolhas feitas durante a aventura
...
while event != 'Exit':              #A janela criada fica aberta até que o botão "Exit" seja apertado
    ...
    if t=='inicio':                     #Parte do código que retoma valores iniciais caso o jogador queira jogar
        ...
        caminho = []
        print('Digite 1 para retornar ao começo da história. O caminho trilhado, e o inventário '
              'retornaram ao que eram no início da história.\n')


#################################################CARACTERÍSTICAS#################################################
    if event == 'Caminho':                              #Parte do código que guarda as escolhas feitas pelo jogador
        if caminho == []:
    ...
    if event == 'Inventário':                           #Parte do código que armazena os itens que o jogador tem
...
#tário acrescentando ou removendo objetos que o jogador poderia usar durante a história.
    ...
    if event == 'Seguir' and t == '3':
        inventario.remove('Bússola')            #O item "Bússola" é removido do inventário
    ...
    if event == 'Seguir' and t == '5':
        ...
        print('[5] Você está muito eufórico com sua condecoração e decide não aceitar sua missão, '
              ...
              'quatro clérigos do templo de Amiz, para a missão. Você resolve dar um passeio '
              'pelo templo e depois de um tempo, entediado vai para os seus aposentos. Dois '
              'dias depois você recebe a notícia que Heragon foi emboscado e morto por ladrões '
    ...
    if event == 'Seguir' and t == '6':
        ...
        print('[6] Claro! Sendo um filho de um exímio caçador, escalar algumas árvores em busca '
              ...
              'sorte um viajante o encontrou caído, nu, em meio à mata e o trouxe para o templo. '
              'A aventura acabou!\n')
    ...
    if event == 'Seguir' and t == '8':
        ...
        if 'Tocha' in inventario:
            print('[8] De posse de uma tocha e filho de um grande caçador você sabe que perigos como esses '
                  ...
                  'escudo ou com a maça digite 19.\n')
        else:
    ...
    if event == 'Seguir' and t == '14':
        caminho.append('14')
        print('[14] Sua reputação de guerreiro nunca foi tão boa assim. Seria abusar da sorte lutar de posse de um '
              'escudo, uma tocha e sua maça com apenas duas mãos. Você coloca o escudo nas costas e segue com '
              'sua maça na mão direita e a tocha na mão esquerda. A criatura percebendo que você dispõe de '
              'uma tocha e a está perseguindo parece ter decidido fugir. Contudo, você quer saber de que se '
              'trata e tirar satisfações. Você, mesmo de placas, se move com certa facilidade pela mata, '
              'habilidade herdade desde a infância. Em certo ponto, já próximo à criatura a mesma desaparece '
              'o que indica uma provável emboscada. Em alerta você caminha cuidadosamente pela mata.\n'
              '“HURRAAA”\n'
    ...
    if event == 'Seguir' and t == '17':
        ...
        print('[17] Você se lembra de conselhos de seu pai de nunca andar a noite sem uma tocha ou lampião. Além disso, '
              ...
              'árvores e quiser procurar por materiais oleaginosos em terra, mesmo que isto gaste mais tempo, digite 9. Se acha que uma tocha '
              'não é importante e que uma fogueira, ao acampar já é o suficiente, digite 10.\n')
    ...
    if event == 'Seguir' and t == '19':
        caminho.append('19')
        print('[19] Você decide não abrir mão de nenhum de seus itens, pois nem ao menos sabe como seu inimigo '
              'irar agir. Mesmo desconfortável com tanta coisa nas mãos você caminha em direção ao invasor. '
              'À medida que caminha o inimigo se afasta mais e mais parecendo querer que você a siga. Mas isto '
              'não é problema. A tocha que têm em mãos irá durar quase uma hora, tempo suficiente para você '
              'encontra-lo e derrota-lo. Devido aos conhecimentos de caça passados por seu pai você começa a '
              'seguir o rastro da criatura a deixando sem opções a não ser lutar. Você está cada vez mais próximo '
              'e um ataque da mesma parece eminente.\n'
              '“HURRAAAA”\n'
              'Esse é o som proveniente de um urro dado pela criatura ao aparecer repentinamente de trás de uma '
              'árvore. Mas você estava atento e com certa dificuldade consegue aparar o golpe. Você desfere um '
              'golpe meio desajeitado devido a está tentando segurar a tocha, a maça e o escudo com apenas duas '
              'mãos. A criatura, cujas feições se assemelham às de um suíno, apara seu golpe com bastante '
              'agressividade e o impacto faz com que você deixe a tocha cair. A criatura se aproveita do descuido '
              'e a chuta para longe o deixando em maus lençóis. Você dá alguns golpes no escuro, mas nenhum '
              'acerta a criatura. Parece que ela aproveitou o seu descuido e fugiu. Você caminha até a tocha...\n'
              '“CRUSHI”\n'
              'Este é o barulho de uma espada perfurando o seu peito. Parece que a criatura o surpreendeu '
              'enquanto você se aproximava da tocha. O golpe lhe deixa sem forças para lutar. O inimigo sorri '
              'para você enquanto seu sangue escorre pela lâmina de sua espada. Não há mais nada o que fazer. '
              'A aventura acabou!\n')
